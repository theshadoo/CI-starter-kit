...
class MY_Lang extends CI_Lang {
	...
	***************************************************/
	...
	function __construct()
	{
		...
		// load config file language i18n
		$CFG->load('i18n');

		// load config file i18n
		$i18nLang = $CFG->item('i18n');

		// stock in property config data i18n
		$this->languages = $i18nLang['iso'];

		// specials uri in config file
		$this->special 	= $i18nLang['special'];

		// default URI redirect if no language
		$this->default_uri = $RTR->routes['default_controller'];
	...
	function default_lang()
	{
		...
		$langConfig = $CFG->item('language');
		if($isoLang = array_search($langConfig, $this->languages))
		{
			return $isoLang;
		}
		return false;
